...
-   `get_system_EBrepo_dirs( osarch, stack_name, stack_version )`: Get all relevant 
    system EBrepo directories for the given architecture, stack name and stack version,
    so including the more generic architectures that are relevant for that stack,
    with the most generic one first.

    The list should match with the list of module directories generated by
    `get_system_module_dirs`.
...
-   `get_user_EBrepo_dirs( osarch, stack_name, stack_version )`: Get the user Easybuild
    repo directories for the given architecture, stack name and stack version,
    so including the more generic architectures that are relevant for that stack,
    with the most generic one first.

    The list should match with the list of module directories generated by
    `get_user_module_dirs`.
